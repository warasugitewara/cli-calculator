...
2. `parseTerm` - 乗除モジュロ＆暗黙乗算
3. `parsePower` - べき乗（高優先度）＆暗黙乗算
...
`2log(2)`, `2pi`, `3sqrt(4)`, `2(3+4)` など、乗算演算子を省略した表記を自動処理：

```cpp
// parseTerm での暗黙乗算
if (token.type == TokenType::FUNCTION || 
    (token.type == TokenType::LPAREN && pos > 0)) {
...
先読み（lookahead）を使用して、eの後ろに数字があるかチェック。

### 3. UTF-8 ルート記号の前処理

`√` 記号（UTF-8: E2 88 9A）を自動検出し、`sqrt` に変換するプリプロセッシング：
